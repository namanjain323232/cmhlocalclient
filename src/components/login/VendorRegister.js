...
import { createOrUpdateUser, createOrUpdateVendor } from "../../actions/auth";
...
const VendorRegister = ({ history }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    toast.success(
      `Email is send to ${email}. Please check your email to complete registration.`
    );
    ...
    await auth.signInWithPopup(googleAuthProvider).then(async (result) => {
      ...
      createOrUpdateVendor(idTokenResult.token)
        .then((res) => {
          ...
          window.alert("vendor regester check");
          var userval = {
            name: res.data.name,
            email: res.data.email,
            token: idTokenResult.token,
        ...
        <div className="col-md-8 offset-md-3">
          <div className="card register-form">
            ...
            <Button
              ...
              icon={<GoogleOutlined />}
              size="large"
              className="mb-2 mt-2 font-weight-bold question"
            >
              Login with Google
            </Button>
            <Button
